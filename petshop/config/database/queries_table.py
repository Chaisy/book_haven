...
class DatabaseQueries:
    ...
    async def get_good_by_animal(animal_get: str) -> Optional[List[dict]]:
        ...
        query = """
                ...
                FROM Books g
                LEFT JOIN Status a ON g.StatusId = a.Id 
                WHERE a.Type = $1;
            """
        try:
    ...
    async def create_user(login: str, password: str, name: str) -> Optional[UserProfile]:
        default_role_id = '6a7cbdcf-b7c4-4f65-9f44-f7223dd8cc55'
        query = """
    ...
    async def delete_animal(animal_id: UUID) -> bool:
        query = """
            DELETE FROM Status WHERE Id = $1 RETURNING Id;
        """
        try:
            result = await Database.fetchrow(query, animal_id)
            return result is not None  
    ...
    async def get_firm(firm_id: UUID) -> Optional[dict]:
        query = "SELECT * FROM Rang WHERE Id = $1;"
        try:
    ...
    async def delete_firm(firm_id: UUID) -> bool:
        query = """
            DELETE FROM Rang WHERE Id = $1 RETURNING Id;
        """
        try:
            result = await Database.fetchrow(query, firm_id)
            return result is not None  
    ...
    async def create_cart(user_id: UUID) -> Optional[dict]:
        query = """
            INSERT INTO Likes (UserId, Books)
            VALUES ($1, $2)
            RETURNING Id, UserId, Books;
        """
        try:
    ...
    async def add_good_to_cart(cart_id: UUID, good_id: UUID) -> Optional[dict]:
        ...
        query_check_good = "SELECT 1 FROM Books WHERE Id = $1 LIMIT 1;"
    ...
    async def get_order_by_id1(order_id: UUID):
        query = "SELECT * FROM Meets WHERE Id = $1"
        result = await Database.fetchrow(query, order_id)
        return result

    @staticmethod
    async def delete_order(order_id: UUID):
        query = "DELETE FROM Meets WHERE Id = $1 RETURNING Id"
        try:
    ...
    async def add_good_to_order(user_id: UUID, good_id: UUID) -> Optional[dict]:
        ...
        query_get_order = """
            SELECT Id
            FROM Meets
            WHERE UserId = $1 AND cardinality(Books) > 0;
        """
        ...
        if not existing_order_id:
            create_order_query = """
                INSERT INTO Meets (UserId, Books)
    ...
    async def get_all_goods() -> List[Good]:
        query = """
                SELECT g.Id, g.Title, g.RangId, g.CategoryOfGoodId, g.StatusId,  g.ImageURL
                FROM Books g;
            """
        try:
            results = await Database.fetch(query)
    ...
    async def update_good(good_id: UUID, good_update: GoodUpdate) -> Optional[Good]:
        ...
        query_get_current = "SELECT * FROM Books WHERE Id = $1;"
        ...
        if good_update.firmId:
            ...
            if not firm_exists:
                raise HTTPException(status_code=400, detail="Rang not found")
        ...
        query_update = """
            UPDATE Books
            SET Title = $1, RangId = $2, CategoryOfGoodId = $3, StatusId = $4, Price = $5, ImageURL = $6
            WHERE Id = $7
            RETURNING Id, Title, RangId, CategoryOfGoodId, StatusId, Price, ImageURL;
        """
        try:
    ...
    async def check_firm_exists(firm_id: UUID) -> bool:
        query = "SELECT 1 FROM Rang WHERE Id = $1;"
        try:
            result = await Database.fetchrow(query, firm_id)
            return result is not None
        except Exception as e:
    ...
    async def check_animal_exists(animal_id: UUID) -> bool:
        query = "SELECT 1 FROM Status WHERE Id = $1;"
        try:
            result = await Database.fetchrow(query, animal_id)
            return result is not None
        except Exception as e:
    ...
    async def add_good(good: GoodCreate) -> Optional[Good]:
        ...
        query = """
            INSERT INTO Books (Title, RangId, CategoryOfGoodId, StatusId, Price, ImageURL)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING Id, Title, RangId, CategoryOfGoodId, StatusId, Price, ImageURL;
        """
        try:
